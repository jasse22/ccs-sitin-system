...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; }
        body { font-family: 'Plus Jakarta Sans', sans-serif; color: #1e2a38; padding: 30px; }
        ...
        thead th { background: #1e3a5f; color: #fff; padding: 10px 12px; text-align: left; font-size: 11px; text-transform: uppercase; letter-spacing: 0.05em; }
...
<div class="no-print" style="text-align:center;margin-bottom:20px;">
    <button onclick="window.print()" style="padding:10px 30px;border:none;border-radius:6px;background:#1e3a5f;color:#fff;font-size:14px;font-weight:600;cursor:pointer;">🖨️ Print / Save as PDF</button>
    <a href="admin_dashboard.php?page=reports" style="margin-left:10px;padding:10px 30px;border:none;border-radius:6px;background:#6c757d;color:#fff;font-size:14px;font-weight:600;text-decoration:none;">← Back</a>
</div>
...
<div class="header">
    ...
    <table>
        ...
        <tbody>
            <?php $cnt = 0; foreach ($records as $r): $cnt++;
                $duration = '';
                if (!empty($r['login_time']) && !empty($r['logout_time'])) {
                    $login = new DateTime($r['login_time']);
                    $logout = new DateTime($r['logout_time']);
