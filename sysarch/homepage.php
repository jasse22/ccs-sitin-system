...
// Fetch the latest 5 items combined from notifications and announcements
$query = "
    SELECT message, created_at FROM notifications WHERE student_id = ?
    UNION
...
.notif-msg{
    ...
    color:#1e2a38;
    font-weight:500;
    line-height: 1.4;        /* Improves readability between lines */
...
<nav>
  ...
  <div class="nav-links">
    <div class="notif-wrapper">
        <a onclick="toggleDropdown()">Notifications
          <span id="notif-badge" class="notif-badge" style="<?= ($unread_count > 0) ? 'display:flex;' : 'display:none;'; ?>">
                <?= $unread_count ?>
            </span>
        </a>
...
// Toggle Display
    if (dropdown.style.display === 'block') {
        ...
        // Hide badge visually when opened
        if (badge) {
            badge.style.display = 'none';
        }
        
        // Call the PHP file to mark as read in the database
        fetch('mark_read.php');
    }
}
...
window.onclick = function(event) {
    if (!event.target.matches('.notif-wrapper a')) {
        var dropdown = document.getElementById('notif-dropdown');
        if (dropdown.style.display === 'block') {
            dropdown.style.display = "none";
        }
